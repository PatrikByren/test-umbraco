...
using UmbracoProject1.Busniess.Models;

namespace UmbracoProject1.Busniess.ScheduleJobs
{
    public class TTImport : RecurringHostedServiceBase
    {
        ...
        public override Task PerformExecuteAsync(object? state)
        {
            ...
            var pressReleasePage = _contentService.GetByLevel(2).FirstOrDefault(x => x.Name == "pressrelease");
            ...
            {
                var content =
                umbracoContextReference?.UmbracoContext?.Content?.GetById(pressReleasePage.Id);
                if (content != null)
                {
                    //BloggLista = content.ChildrenOfType("articleList").FirstOrDefault();
                    BloggLista = content;
                ...
                if (BloggLista != null)
                {
                    ...
                    foreach (var item in json)
                    {
                        ...
                        if (!hasMatch)
                        {
                            ...
                            var demoproduct = _contentService.Create(item.Id.ToString(), parentId,
                            "pressItem");
							...
							demoproduct.SetValue("pressItemId", item.Id.ToString());
